...
function Index(){
  ...
  const { data, isError } = getProductById(ProductId);
  const {data: isInCart ,isError: isInCartError} = isAlreadyInCart(ProductId)

  
  
  useEffect(()=>{
    if(data)
    axios.post('/api/product/getSameAsCategory',{'category':data.category,'currentPostId':data.id})
          .then(res=>{
  ...
  if (isError) {
    ...
    const handleToCart = () =>{
      ...
      if(!isInCart.alreadyInCart){
        ...
        axios.post('/api/product/toCart',{"quantity":cartData.quantity,"productId":data.id})
        .then(res=>{
          router.push('/cart')
  ...
  return (
          ...
          <div className="flex justify-start flex-col items-start lg:basis-1/2 gap-[24px] h-fit">
            ...
            <div className="flex flex-col gap-[16px]">
              ...
              <div className="flex flex-col gap-[8px]">
                ...
                <div className="font-bold text-[32px]">{data.title}</div>
                <p className="text-[14px] text-neutral-500">{data.category}</p>
                <div className="text-[#343434] font-normal text-[32px]">
                  $ {data.price}
                </div>
              </div>

              <div className="text-[#34343499]">{data.description}</div>
            </div>
            ...
            />
              ...
              <SecondaryButton
                label={ isInCart ? isInCart.alreadyInCart ? 'Remove from cart' :`Add to Cart` : '...Loading'}
                outline
                full
        ...
        body={(
          <div className="2xl:w-[80%] md:w-full mx-auto">
            ...
            <div className="bg-white rounded-lg p-[24px] flex flex-col gap-10  w-full ">
                <div className="">
                  <h3 className=" text-main font-semibold">Weekly Watering</h3>
                ...
                <div className="">
                  <h3 className=" text-main font-semibold">Light Requirements</h3>
                  <p className=" text-gray-main font-light">
   ...
   {recommended ? 
    <Container>
      <h2 className="font-medium text-[26px] text-center pb-10">Recomended Plants</h2>
